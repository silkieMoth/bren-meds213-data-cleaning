...
snowsurvey_fixed <- snowsurvey_csv %>%
  mutate(across(Snow_cover:Total_cover, ~ ifelse(.x == ".", NA, .x)))
...
```{r}
snowsurvey_csv %>% 
  # filter(if_any(matches("-"), ~ .x == TRUE))
  filter_all(any_vars(. == "-"))
  # filter(grepl('-', Snow_cover))
```
...
```{r}
snowsurvey_fixed <- snowsurvey_fixed %>% 
  mutate(across(everything(), ~ ifelse(.x == "unk", NA, .x)))
```
...
```{r}
snowsurvey_csv %>%
  filter_at(vars(Snow_cover:Total_cover), all_vars(. > 100))
...
```{r}
snowsurvey_fixed <- snowsurvey_fixed %>% 
  mutate(across(Snow_cover:Total_cover, ~ as.numeric(.x)))
...
```{r}
 snowsurvey_fixed %>% 
  select(Snow_cover:Total_cover) %>% 
  filter(if_any(everything(), ~ . > 100 | . < 0)) 
...
snowsurvey_fixed <-  snowsurvey_fixed %>% 
  select(Snow_cover:Total_cover) %>% 
  filter(if_any(everything(), ~ . > 100 | . < 0)) %>% 
  mutate(across(everything(), ~ ifelse(. > 100 | . < 0, NA, .)))
```
...
#  filter(if_any(Alder_Flycatcher:Yellow_rumped_Warbler, ~ . != 0 | . != 1)) %>% 
  mutate(across(Alder_Flycatcher:Yellow_rumped_Warbler, ~ ifelse(. != 0, 1, 0))) %>% 
  mutate(across(Alder_Flycatcher:Yellow_rumped_Warbler, ~ as.numeric(.))) %>% 
  # mutate(across(Alder_Flycatcher:Yellow_rumped_Warbler, ~ as.numeric(.x))) %>%
  pivot_longer(cols = Alder_Flycatcher:Yellow_rumped_Warbler,
               names_to = "Species",
               values_to = "Presence")
...
```{r}
species_csv %>% 
  select(Alder_Flycatcher:Yellow_rumped_Warbler) %>% 
...
species_presence <- species_presence %>% 
  select(-Num_observers, -All_obs_reported, -Observer_hours)
```
