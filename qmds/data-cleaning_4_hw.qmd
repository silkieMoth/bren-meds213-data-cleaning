...
```{r}
write_csv(snowsurvey_fixed, file.path(datadir_processed, "snow_cover_hw.csv"))
```
...
```{r}
write_csv(species_presence, file.path(datadir_processed, "species_presence_hw.csv"))
```
